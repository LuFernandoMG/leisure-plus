...
const Showcase: React.FC<ShowcaseProps> = ({ data, type }) => {
  ...
  const fetchElements =
    type === "movies"
      ? moviesStore((state: any) => state.fetchMovies)
      : tvStore((state: any) => state.fetchShows);
  const elements =
    type === "movies"
      ? moviesStore((state: any) => state.movies)
      : tvStore((state: any) => state.shows);
  const sort =
    type === "movies"
      ? moviesStore((state: any) => state.sort)
      : tvStore((state: any) => state.sort);
  const setSort =
    type === "movies"
      ? moviesStore((state: any) => state.setSort)
      : tvStore((state: any) => state.setSort);
  const page =
    type === "movies"
      ? moviesStore((state: any) => state.page)
      : tvStore((state: any) => state.page);
  const setPage =
    type === "movies"
      ? moviesStore((state: any) => state.setPage)
      : tvStore((state: any) => state.setPage);
  const sortValues =
    type === "movies"
      ? moviesStore((state: any) => state.sortOptions)
      : tvStore((state: any) => state.sortOptions);
  const getGenderList =
    type === "movies"
      ? moviesStore((state: any) => state.setGenderList)
      : tvStore((state: any) => state.setGenderList);
  const genderList =
    type === "movies"
      ? moviesStore((state: any) => state.genderList)
      : tvStore((state: any) => state.genderList);
  const setFilters =
    type === "movies"
      ? moviesStore((state: any) => state.setFilters)
      : tvStore((state: any) => state.setFilters);
  const filters =
    type === "movies"
      ? moviesStore((state: any) => state.filters)
      : tvStore((state: any) => state.filters);
      ...
      <div className={styles.showcase__header}>
        {isGrid && (
          <div className={styles.filter__toggle}>
            <button className={styles.filter__button} onClick={handleShowFilter}>
              <FaFilter />
            </button>
          </div>
        )}
          ...
          <div className={styles.movies__grid}>
            {elements.map((element: Movie | Show) => (
              <Card
                ...
                key={element.id}
                clean
                root={type === "movies" ? "/movies" : "/series"}
              />
            ))}
